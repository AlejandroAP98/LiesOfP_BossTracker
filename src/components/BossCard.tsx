...
export const BossCard = ({
    ...
    if (running && !derrotados) {
      id = window.setInterval(() => {
        if (startTimeRef.current) {
          ...
          setElapsed((elapsedFromParent || 0) + diff);
        }
      }, 1000);
    }
    return () => {
  ...
  }, [running, derrotados, elapsedFromParent]);
  ...
  const handlePause = () => {
    if (running && startTimeRef.current) {
      const diff = Math.floor((Date.now() - startTimeRef.current) / 1000);
      const total = (elapsedFromParent || 0) + diff;
      setElapsed(total);
      setRunning(false);
      startTimeRef.current = null;
      onTimeChange(boss.name, total);
    }
  };
  ...
  const handleToggle = () => {
    if (running && startTimeRef.current) {
      handlePause(); // pausa y guarda antes de marcar derrotado
    }
      ...
      <div className="card-body w-full flex flex-col justify-between">
        <div className="flex flex-col justify-between w-full gap-1">
          <div className="flex justify-between items-center w-full">
            <div className="text-sm font-medium mx-1">
              <span className="countdown items-center font-mono font-bold text-xl">{formatTime(elapsed)}</span>
            </div>
          ...
          <a href={boss.detail_url} target="_blank" className="btn btn-ghost btn-sm">
            <span className="flex items-center gap-2">Visitar
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-arrow-up-right" viewBox="0 0 16 16">
                <path d="M8.636 3.5a.5.5 0 0 0-.5-.5H1.5A1.5 1.5 0 0 0 0 4.5v10A1.5 1.5 0 0 0 1.5 16h10a1.5.5 0 0 0 1.5-1.5V7.864a.5.5 0 0 0-1 0V14.5a.5.5 0 0 1-.5.5h-10a.5.5 0 0 1-.5-.5v-10a.5.5 0 0 1 .5-.5h6.636a.5.5 0 0 0 .5-.5z"/>
